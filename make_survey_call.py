...
async def make_survey_call(phone_number, question, row_index):
    ...
    metadata = json.dumps(
        {"phone_number": phone_number, "question": question, "row_index": row_index}
    )
...
async def process_survey_calls():
    ...
    for item in data:
        if item["answer"] or (item["status"] and item["status"] != ""):
            logger.info(
                f"Skipping row {item['row_index']} as it already has an answer or status"
            )
            continue

        logger.info(
            f"Processing survey call to {item['phone_number']} with question: {item['question']}"
        )

        await make_survey_call(
            item["phone_number"], item["question"], item["row_index"]
        )
...
async def main():
    ...
    if not outbound_trunk_id:
        logger.error(
            "SIP_OUTBOUND_TRUNK_ID is not set. Please add it to your .env file."
        )
        return
